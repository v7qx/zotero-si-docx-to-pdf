...
notify-skip-no-pdf = 未检测到同一条目下已有其他 PDF 附件，已跳过该附件。
notify-skip-not-ready = DOC / DOCX 附件尚未完成下载或写入，已跳过本次自动转换。
error-invalid-libreoffice-path = 转换失败：请检查 LibreOffice 路径是否选择了 soffice（Windows 为 soffice.exe）或 libreoffice，而不是 swriter 或 soffice_safe。
